...
{    
    ...
    if (i==mX)
    {
        fprintf(f,"%d %d %d %d %d\n",level,p->life,p->score,ammo,blueammo); // /n esr obligatoire pour que les infos suvantes aillent bien à la ligne suivante
        i++;
    }

    if (i ==mX+1)
    { 
        fprintf(f,"%s",playernamearray);
    }
...
void saveScores()
{
	...
	{	
        if (i==0)
        {
            fprintf(f,"%d %d %d %d %d\n",tabScores[0], tabScores[1], tabScores[2], tabScores[3], tabScores[4]);
        }
        if (i>0 )
        {
            fprintf(f,"%s\n",tabNamesScores[i-1]);
